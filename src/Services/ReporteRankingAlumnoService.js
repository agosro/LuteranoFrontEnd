...
export const rankingCurso = async (token, cursoId, anio) => {
  void token
  const data = await httpClient.get(`/api/reportes/ranking-alumnos/curso/${cursoId}?anio=${encodeURIComponent(anio)}`)
...
export const rankingColegio = async (token, anio) => {
  void token
  const data = await httpClient.get(`/api/reportes/ranking-alumnos/colegio?anio=${encodeURIComponent(anio)}`)
...
export const rankingTodosCursos = async (token, anio) => {
  void token
  const data = await httpClient.get(`/api/reportes/ranking-alumnos/todos-cursos?anio=${encodeURIComponent(anio)}`)
