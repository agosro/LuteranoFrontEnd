...
export const generarActa = async (token, payload) => {
  void token
  const data = await httpClient.post('/api/actas/generar', payload)
...
export const actualizarActa = async (token, payload) => {
  void token
  const data = await httpClient.put('/api/actas', payload)
...
export const eliminarActa = async (token, id) => {
  void token
  const data = await httpClient.delete(`/api/actas/${id}`)
...
export const obtenerActaPorId = async (token, id) => {
  void token
  const data = await httpClient.get(`/api/actas/${id}`)
...
export const obtenerActaPorMesa = async (token, mesaId) => {
  void token
  const data = await httpClient.get(`/api/actas/mesa/${mesaId}`)
...
export const buscarActas = async (token, q) => {
  void token
  const data = await httpClient.get(`/api/actas/buscar?q=${encodeURIComponent(q)}`)
...
export const listarActasPorTurno = async (token, turnoId) => {
  void token
  const data = await httpClient.get(`/api/actas/turno/${turnoId}`)
...
export const listarActasPorCurso = async (token, cursoId) => {
  void token
  const data = await httpClient.get(`/api/actas/curso/${cursoId}`)
...
export const listarActasEntreFechas = async (token, desde, hasta) => {
  ...
  const data = await httpClient.get(`/api/actas/entre-fechas?${params}`)
